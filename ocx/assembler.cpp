...
void CAssembler::Val (CString chaine, int& resultat, int& erreur)
{
	...
	if (chaine[0]=='$')
		resultat = strtol (chaine.Right(chaine.GetLength()-1), &stopstring, 16);
	else
		resultat = strtol(chaine, &stopstring, 10);
	erreur = errno;
}
...
void CAssembler::DoErreur (int n)
{
	...
	switch (n)
	{
     ...
     ErreurInfos.Message = mess+" dans "+(CString)34+linebis+(CString)34;
	 Erreur = true;
     throw "Erreur !";
}
