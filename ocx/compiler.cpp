	...
	while ((lexeme.Type == ID) || (IsKeyWord("VAR") && (Fonc)))
	{
		...
		if (IsKeyWord("ARRAY"))
		{
			...
			while (true)
			{
				...
				Type.Debut[Type.Dimension] = StrToInt(lexeme.Valeur);
				LireLexeme ();
				if (!IsKeyWord("..")) throw ".. attendu !";
				LireLexeme ();
				if (lexeme.Type!=ENTIER) throw "Nombre entier attendu !";
				Type.Fin[Type.Dimension] = StrToInt(lexeme.Valeur);
				LireLexeme ();
				if (!IsKeyWord(","))
				{
					if (!IsKeyWord("]")) throw "] attendu !";
			...
			(IsKeyWord("BOOLEAN")))
		{
			...
			if (IsKeyWord("STRING"))
			{
				...
				if (IsKeyWord("["))
				{
					LireLexeme();
					if (lexeme.Type!=ENTIER) throw "Nombre entier attendu !";
					Type.Fin[Type.Dimension+1] = StrToInt(lexeme.Valeur);
					LireLexeme();
					if (!IsKeyWord("]")) throw "] attendu !";
					LireLexeme();
				}
...
void CCompiler::CompileInstr ()
{
	...
	if (lexeme.Type==ASSEMBLER)
	{
		while (lexeme.Type==ASSEMBLER)
		{
			...
			if (lexeme.Valeur.Find("@")!=-1) throw "Caractère @ interdit !";
