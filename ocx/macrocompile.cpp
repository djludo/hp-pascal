...
void CMacroCompile::Chargement(CLdjTextFile *Fichier)
{
	...
	while (!Fichier->eof())
	{
		...
		chaine += ":";
		if (chaine.Find(":")!=-1) { current.op=chaine.Left(chaine.Find(":")); chaine=chaine.Right(chaine.GetLength()-chaine.Find(":")-1); }
		if (current.op=="DATA") { current.arg1=chaine.Left(chaine.GetLength()-1); chaine = "";}
		
		if (chaine.Find(":")!=-1) { current.arg1=chaine.Left(chaine.Find(":")); chaine=chaine.Right(chaine.GetLength()-chaine.Find(":")-1); }
		if (chaine.Find(":")!=-1) { current.arg2=chaine.Left(chaine.Find(":")); chaine=chaine.Right(chaine.GetLength()-chaine.Find(":")-1); }
		if (chaine.Find(":")!=-1) { current.arg3=chaine.Left(chaine.Find(":")); chaine=chaine.Right(chaine.GetLength()-chaine.Find(":")-1); }
		if (chaine.Find(":")!=-1) { current.arg4=chaine.Left(chaine.Find(":")); chaine=chaine.Right(chaine.GetLength()-chaine.Find(":")-1); }
...
void CMacroCompile::Traduction ()
{
	...
	while (true)
	{
		...
		if (current.op=="CONST") { typCONST = current.arg1; if (typCONST=="STRING") typCONST2 = StrToInt(current.arg2); }
...
void CMacroCompile::TraduitCONST(CMacroCompile::TInstruction current)
{
	...
	if (typCONST=="BYTE") SendNoOptimize ("HEX "+ReverseHexa(HexaChar(StrToInt(current.arg1))));
	if (typCONST=="INTEGER") SendNoOptimize ("HEX "+ReverseHexa(HexaInteger(StrToInt(current.arg1))));
	if (typCONST=="REAL") SendNoOptimize ("HEX "+ReverseHexa(HexaFloat(StrToFloat(current.arg1))));
	...
	if (typCONST=="STRING") 
	{
		...
		SendNoOptimize ("ASC "+(CString)34+current.arg1+(CString)34);
...
void CMacroCompile::TraduitProc (CMacroCompile::TInstruction current)
{
	...
	SendNoOptimize (":@F@"+ProcName);
	 ...
	 SendNoOptimize ("LC(5) "+HexaInteger(StrToInt(current.arg2)+StrToInt(current.arg3)));
     SendNoOptimize ("A=A-C A");
	 SendNoOptimize ("C=R1.F A");
	 SendNoOptimize ("D=C A");
     SendNoOptimize ("R1=A.F A");	 
	 SendNoOptimize ("LC(5) "+HexaInteger(StrToInt(current.arg2)+StrToInt(current.arg3)+StrToInt(current.arg4)));
	 ...
	 SendNoOptimize ("LA(5) "+HexaInteger(StrToInt(current.arg2)));
	 SendNoOptimize ("C=R1.F A");
	 SendNoOptimize ("A=C+A A");
...
void CMacroCompile::TraduitCST(CMacroCompile::TInstruction current)
{
	...
	if (current.arg1=="BYTE") Send ("LA(2) "+HexaChar(StrToInt(current.arg2)));
	if (current.arg1=="INTEGER") Send ("LA(5) "+HexaInteger(StrToInt(current.arg2)));
	if (current.arg1=="REAL") Send ("LA(16) "+HexaFloat(StrToFloat(current.arg2)));
